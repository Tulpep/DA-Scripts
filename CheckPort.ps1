...
function Check-Port($PC, $Port, $Protocol)
{

    switch ($Protocol.ToUpper())
    {
        "TCP" {$Result = Test-Port -computer $PC -port $Port -TCP}
        "UDP" {$Result = Test-Port -computer $PC -port $Port -UDP}
        default {throw "Protocolo Invalido"}
    }
